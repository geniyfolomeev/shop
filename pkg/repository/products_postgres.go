...
func (r *ProductsListPostgres) GetAllProducts() ([]shop.Product, error) {
	...
	query := fmt.Sprintf("SELECT title, price, description, category, encode(image, 'base64') as image, rating FROM %s", productsTable)
...
func (r *ProductsListPostgres) GetProductById(id int) (shop.Product, error) {
	...
	query := fmt.Sprintf("SELECT title, price, description, category, encode(image, 'base64') as image, rating FROM %s WHERE id=$1", productsTable)
